...
void topic(char buf[BUFFER_LEN],int i,int sockfd){
	...
	int index;
	bool show=false;
	for(int k=6;(buf[k]!=' ')&&(buf[k]!='\n') ;k++){
		channel_name+=buf[k];
		index=k;
	}
	cout<<strlen(buf)<<endl;
	cout<<index<<endl;
	if(strlen(buf)==(index+2))
		show=true;
	cout<<channel_name<<endl;
	if(show){
		cout<<"show topic"<<endl;
		//show the topic
		string tpc;
		for(int p=0;p<ch_list.size();p++){
			if(ch_list[p].ch_name==channel_name){
				tpc=ch_list[p].topic;
				cout<<"find topic: "<<tpc<<endl;
				break;
			}
		}
		char tmp[1024];
	...
	cout<<"add to channel list\n";
	cout<<"==DEBUGGING topic=="<<endl;
	for(int j=0;j<ch_list.size();j++){
		cout<<"list name:"<<ch_list[j].ch_name<<endl;
		cout<<"channel name:"<<channel_name<<" topic: "<<topic_str<<endl;
		cout<<typeof(ch_list[j].ch_name)<<" "<<typeof(channel_name)<<endl;
		if(ch_list[j].ch_name==channel_name){
...
void privmsg(char buf[BUFFER_LEN],int i,int sockfd){
	...
	cout<<"==END DEBUGGING privmsg=="<<endl;
	char temp[1024];
	string message_buffer;
		...
		for (i = 0; i <= maxi; i++) {	/* check all clients for data */
			...
			if (FD_ISSET(sockfd, &rset)) {
				...
				if(cmd.substr(0,4)=="LIST"){
					list(buf,client[i].id,sockfd);
				}
				/*JOIN <channel>*/
				if(cmd.substr(0,4)=="JOIN"){
					join(buf,client[i].id,sockfd);
				}
				/*TOPIC <channel> [<topic>]*/
				if(cmd.substr(0,5)=="TOPIC"){
					topic(buf,client[i].id,sockfd);
				}
				/*NAMES [<channel>]*/
				/*PART <channel>*/
				if(cmd.substr(0,4)=="PART"){
					part(buf,client[i].id,sockfd);
				}
				/*USERS*/
				if(cmd.substr(0,5)=="USERS"){
					users(buf,client[i].id,sockfd);
				}
				/*PRIVMSG <channel> <message>*/
				if(cmd.substr(0,7)=="PRIVMSG"){
					privmsg(buf,client[i].id,sockfd);
				}
				/*QUIT*/
				if(cmd.substr(0,4)=="QUIT"){
					quit(client[i].id,sockfd);
					close(sockfd);
